...
public class ChatController {
	...
	@RequestMapping(value="/send_old", method=RequestMethod.POST)
	public String processReceivedMessage_Old(@RequestParam(value="chatMessage", required=true) String message,
											@ModelAttribute("user") User user) {
	...
	@RequestMapping(value="/poll_old", method=RequestMethod.POST)
	public @ResponseBody Map<Integer, ChatMessage> pollForMessages_Old
				(@RequestParam(value="lastMessageId", required=true) int lastMessageId, @ModelAttribute("user") User user) {
		
		//log.info("Received lastMessageId=" + lastMessageId);
		Map<Integer, ChatMessage> messageMap 
				= lastMessageId == -1 ? processFirstTimePoll_Old(user) : processRegularPoll_Old(user, lastMessageId);
		return messageMap;
	}
	
	
	private Map<Integer, ChatMessage>  processRegularPoll_Old(User user, int lastMessageId) {
		Map<Integer, ChatMessage> messageMap = new HashMap<Integer, ChatMessage>();
		// TODO what about zero-range check?
	...
	private Map<Integer, ChatMessage> processFirstTimePoll_Old(User user) {
